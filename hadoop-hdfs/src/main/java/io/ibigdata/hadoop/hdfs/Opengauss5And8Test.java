...
public class Opengauss5And8Test {
    ...
    public static void test8oracle() throws SQLException, ClassNotFoundException{
        String jdbcURL = "jdbc:opengauss://10.20.194.39:8000/jres_oracle";
        String username = "jres";
        String password = "jres123!";

        String clearSql = "truncate table muti_types001_d";
        String writeRecordSql = "INSERT INTO muti_types001_d (c_int,c_tinyint,c_smallint,c_integer,c_bigint,c_number,c_boolean,c_clob,c_blob,c_bytes) VALUES(?,?,?,?,?,?,?,?,?,?)";
        String metaDataSql = "select c_int,c_tinyint,c_smallint,c_integer,c_bigint,c_number,c_boolean,c_clob,c_blob,c_bytes from muti_types001_d where 1=2";
        ...
        try(Connection connection = DriverManager.getConnection(jdbcURL,username,password);
            ...
            ResultSet rs = stmtQuery.executeQuery("select c_int,c_tinyint,c_smallint,c_integer,c_bigint,c_number,c_boolean,c_clob,c_blob,c_bytes from muti_types001_d");
            System.out.println("---------------打印数据----------------------");
            while (rs.next()) {
                System.out.println( rs.getObject(1) + "      " + rs.getObject(2) + "      " + rs.getObject(3)
                        + "      " + rs.getObject(4) + "      " + rs.getObject(5) + "      " + rs.getObject(6)
